...
public class PassCheck {
	...
	private String checkPassword(String pwd, String id){
		  ...
		  // 비밀번호 포맷 확인(영문, 특수문자, 숫자 포함 8자 이상 15자 이하)
		  Pattern passPattern1 = Pattern.compile("^(?=.*[a-zA-Z])(?=.*\\d)(?=.*\\W).{8,15}$");
		  Matcher passMatcher1 = passPattern1.matcher(pwd);
		  
		  if(!passMatcher1.find()){
		    return "비밀번호는 영문과 특수문자 숫자를 포함하며 8자 이상 15자 이하로 입력 해야합니다.";
		  }
	...
	// 외부와 연결되는 메소드
	public String paPw(String pw, String id) {
		return checkPassword(pw,id);
	}
}
